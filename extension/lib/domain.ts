...
export function liveBookmarks(profile: ProfileData) {
  return profile.bookmarks.filter(isVisible).sort((a, b) => Number(a.json.sort_index ?? 0) - Number(b.json.sort_index ?? 0))
}
...
export function saveBookmark(profile: ProfileData, draft: BookmarkDraft) {
  ...
  const id = uid()
  const result = addBookmarkRecord(profile.lists, profile.bookmarks, { ...draft, url, icon: draft.icon || faviconFor(url), tags: cleanTags(draft.tags) }, id, now())
...
export function tombstone<T extends { id: string; json: Record<string, unknown>; updatedAt: string }>(rows: T[], pending: string[], id: string) {
...
export const mergeRows = <T extends { id: string; updatedAt: string }>(local: T[], remote: T[], pending: string[]) => mergeSyncRows(local, remote, new Set(pending))

export function reorder<T extends { id: string; json: Record<string, unknown>; updatedAt: string }>(rows: T[], ids: string[], pending: string[]) {
  const timestamp = now()
