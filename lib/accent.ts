 ...
 */
export const onAccentChannels = (accent: AccentId): string => {
  const ramp = accentRamp(accent)
  const fill = channelsToRgb(ramp[ANCHOR_INDEX]!)
...
export const accentVariables = (accent: AccentId): Record<string, string> => {
  ...
  return {
    ...
    '--nori-accent-on': onAccentChannels(accent),
  }
}
...
export const onAccentColor = (accent: AccentId): string =>
  `rgb(${onAccentChannels(accent).split(' ').join(', ')})`
